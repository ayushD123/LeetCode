...
public:
    ...
    void create_tree(node* root, vector<int> &v){
        //Your code goes here
        root->data=v[0];
        root->left=newNode(v[1]);
         root->right=newNode(v[2]);
          root->left->left=newNode(v[3]);
           root->left->right=newNode(v[4]);
            root->right->left=newNode(v[5]);
             root->right->right=newNode(v[6]);
    }
